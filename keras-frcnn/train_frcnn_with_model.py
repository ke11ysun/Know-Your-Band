...
parser.add_option("--config_out_filename", dest="config_out_filename", help=
"Location to store all the metadata related to the training (to be used when testing).",
                  default="config2.pickle")

parser.add_option("--config_in_filename", dest="config_in_filename", default="config_init.pickle")
...
config_input_filename = options.config_in_filename

with open(config_input_filename, 'rb') as f_in:
    C = pickle.load(f_in)
print("Previous config file " + config_input_filename + " has been loaded.")
...
config_output_filename = options.config_out_filename
...
model_all2 = load_model('./model_all_init.h5', custom_objects={'RoiPoolingConv': RoiPoolingConv, 'FixedBatchNormalization': FixedBatchNormalization})
model_rpn2 = load_model('./model_rpn_init.h5', custom_objects={'RoiPoolingConv.py': RoiPoolingConv,
...
model_classifier2 = load_model('./model_classifier_init.h5', custom_objects={'RoiPoolingConv': RoiPoolingConv,
                                                                         'FixedBatchNormalization': FixedBatchNormalization,
...
losses = np.load('losses_init.npy')
rpn_accuracy_rpn_monitor = []
rpn_accuracy_for_epoch = []
start_time = time.time()

# best_loss = np.Inf
best_loss = np.load('best_loss_init.npy')
...
for epoch_num in range(num_epochs):
    ...
    while True:
        try:
            ...
            loss_rpn = model_rpn2.train_on_batch(X, Y)

            P_rpn = model_rpn2.predict_on_batch(X)
            ...
            loss_class = model_classifier2.train_on_batch([X, X2[:, sel_samples, :]],
            ...
            if iter_num == epoch_length:
                ...
                if curr_loss < best_loss:
                    ...
                    best_loss = curr_loss
                    model_all2.save_weights(C.model_path)

                    model_all2.save('./model_all2.h5')
                    model_classifier2.save('./model_classifier2.h5')
                    model_rpn2.save('./model_rpn2.h5')
                    np.save('losses2.npy', losses)
                    np.save('best_loss2.npy', best_loss)

                    print("Best weights so far saved to " + C.model_path + ". best_loss = " + str(best_loss))
                    print(losses)
